...
(function($, undefined) {
	var fixAttr = function(target, clone) {
	...
	}, doc = document, mallan = $.dom.mallan, node = {
		parent : function() {
			var parents = new mallan();
			this.each(function() {
		...
		append : function(content) {
			if( typeof content === 'string') {
				...
				this.append(new mallan(div.childNodes));
			}
			...
			else if(content.mallantype === "mallan") {
				//mallan object
				this.each(function(i) {
					var self = this;
					if(i === 0) {
		...
		insertBefore : function(el) {
			el = el.mallanType === 'mallan' ? (el[0] ? el[0] : null) : (el.nodeType ? el : null);
			if(el) {
				this.each(function() {
	...
	$.dom.mallan.extend(node);
	$.nameSpace.pack("Mallan.dom.mallan.Node", node);
})(Mallan);