...
(function($, undefined) {
	...
	var doc = document, styleExcept = {
		'float' : $.dom.mallan.support.cssFloat ? 'cssFloat' : 'styleFloat',
			...
			if(arguments.length == 1) {
				if( typeof key === "string") {
					...
					this.each(function() {
						for(var o in key) {
							if(o === 'opacity') {
								$.dom.mallan.support.opacity ? this.style[o] = key[o] : this.style.filter = ('alpha(opacity=' + key[o] * 100);
							}
				...
				this.each(function() {
					if(key === 'opacity') {
						$.dom.mallan.support.opacity ? this.style[key] = value : this.style.filter = ('alpha(opacity=' + value * 100);
					}
					else {
	...
	$.dom.mallan.extend(style);
	$.nameSpace.pack("Mallan.dom.mallan.Style", style);
})(Mallan);
